...
void GcodeSuite::M703() {
  ...
  // Show initial "wait for cold pull" message
  #if HAS_LCD_MENU
    lcd_pause_show_message(PAUSE_MESSAGE_COLDPULL, PAUSE_MODE_COLD_PULL_FILAMENT, target_extruder);
  #endif
  ...
  // Unload filament
  #if ENABLED(PRUSA_MMU2)
    mmu2.unload(); //don't wanna support cold pull for this so just normal unload
  #else
    #if EXTRUDERS > 1 && ENABLED(FILAMENT_UNLOAD_ALL_EXTRUDERS)
      if (!parser.seenval('T')) {
        HOTEND_LOOP() {
          if (e != active_extruder) tool_change(e, false);
          cold_pull_filament(true, PAUSE_MODE_COLD_PULL_FILAMENT);
        }
      }
      else
    #endif
