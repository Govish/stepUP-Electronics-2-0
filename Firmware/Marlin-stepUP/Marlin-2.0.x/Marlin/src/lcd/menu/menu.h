...
class MenuItem_gcode : public MenuItem_button {
  public:
    ...
    static void action(PGM_P const, const char * const pgcode, bool sync = false);
    static inline void action(PGM_P const pstr, const uint8_t, const char * const pgcode, bool sync=false) { action(pstr, pgcode, sync); }
};
...
#define GCODES_ITEM_SYNC(LABEL, GCODES)               MENU_ITEM_P(gcode, GET_TEXT(LABEL), GCODES, true)
...
#define GCODES_ITEM_N_SYNC(N, LABEL, GCODES)        MENU_ITEM_N_P(gcode, N, GET_TEXT(LABEL), GCODES, true)
