...
void menu_main() {
  ...
  if (busy) {
    ...
    SUBMENU(MSG_TUNE, menu_tune);
      ...
      if (card_detected) {
        ...
        #if PIN_EXISTS(SD_DETECT)
          STATIC_ITEM(MSG_NO_MEDIA);
        #else
          GCODES_ITEM(MSG_ATTACH_MEDIA, M21_STR);
          STATIC_ITEM(MSG_MEDIA_RELEASED);
        #endif
      }
  ...
  //
    ...
    if (powersupply_on) {
      if(!SHUTDOWN_STAGED())
        GCODES_ITEM_SYNC(MSG_SHUTDOWN_PS, PSTR("M81"));
      else
        GCODES_ITEM_SYNC(MSG_CANCEL_SHUTDOWN_PS, PSTR("M80"));
    }
    else
      GCODES_ITEM_SYNC(MSG_SWITCH_PS_ON, PSTR("M80"));
  #endif
    ...
    if (card_detected) {
      ...
      #if PIN_EXISTS(SD_DETECT)
        STATIC_ITEM(MSG_NO_MEDIA, SS_LEFT);
      #else
        GCODES_ITEM(MSG_ATTACH_MEDIA, M21_STR);
        STATIC_ITEM(MSG_MEDIA_RELEASED, SSLEFT);
      #endif
    }
