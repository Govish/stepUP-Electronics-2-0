...
void MarlinUI::draw_status_screen() {
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    void MarlinUI::update_indicators(bool force) {
      // Set the LEDS - referred to as backlights by the LiquidTWI2 library
      static uint8_t ledsprev = 0;
      uint8_t leds = 0;

      #if ENABLED(ALTERNATE_LEDS)

        #if HAS_HEATED_BED && (HOTENDS > 0)
          if(ledsprev & LED_C) { //if the temperature light is on, i.e. the printer is hot
            if((thermalManager.degBed() < BED_COOL_TEMP) && (thermalManager.degHotend(0) < EXT_COOL_TEMP)); //deactivate temp LED only if cool (with hysteresis)
            else leds |= LED_C; // keep the temp led on
          }
          else { //if the temp light was off, i.e. the printer is cold
            if((thermalManager.degBed() > BED_HOT_TEMP) || (thermalManager.degHotend(0) > EXT_HOT_TEMP)) leds |= LED_C; //turn on the light if hot (with hyst)
          }
        #endif

        #if ENABLED(SDSUPPORT)
          if(card.isMounted()) leds |= LED_B;
        #endif

        if(all_axes_known()) leds |= LED_A;
