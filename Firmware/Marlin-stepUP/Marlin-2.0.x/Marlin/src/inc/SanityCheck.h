...
/**
 * Check the hardware PWM pins for the heater pwm controls
 * hacked together, so only supporting bed heater and hotend 0 heater
 */
#ifdef HARDWARE_PWM_HEATERS
    static_assert(EXTRUDERS <= 1, "Hardware PWM Heaters is only supported for single extruder");
    static_assert(PWM_PIN(HEATER_0_PIN), "Extruder pin is not a PWM pin");
    #ifdef HEATER_BED_PIN
      static_assert(PWM_PIN(HEATER_BED_PIN), "Heated bed pin is not a PWM pin");
    #endif
    static_assert(!(ENABLED(HARDWARE_PWM_HEATERS) && ENABLED(SLOW_PWM_HEATERS)), "HARDWARE_PWM_HEATERS and SLOW_PWM_HEATERS must not both be enabled");
#endif
