...
#if ENABLED(PRINTER_EVENT_LEDS) || ENABLED(ALTERNATE_EVENT_LEDS)
...
// public:
  ...
  void Temperature::set_fan_speed(uint8_t target, uint16_t speed) {
    ...
    NOMORE(speed, 255U);
      ...
      if (target != active_extruder) {
        if (target < EXTRUDERS) {
            ...
            if (singlenozzle_fan_speed[target] != speed) printerEventLEDs.fan_changed();
          #endif
    ...
    #if ENABLED(ALTERNATE_EVENT_LEDS)
      if(fan_speed[target] != speed) printerEventLEDs.fan_changed();
    #endif
...
void Temperature::tick() {
  ...
  #if ENABLED(HARDWARE_PWM_HEATERS)
    #define _WRITE_PWM(N,T) do { \
      AWRITE_HEATER_##N(T.soft_pwm_amount << 1); \
    } while(0)

    #if HOTENDS
      _WRITE_PWM(0, temp_hotend[0]);
    #endif
    #if HAS_HEATED_BED
      _WRITE_PWM(BED, temp_bed);
    #endif
  #endif

  //soft pwm (run the soft pwm heaters only if not hardware pwm)
    ...
    if (pwm_count_tmp >= 127) {
      ...
      #if DISABLED(HARDWARE_PWM_HEATERS)
        #if HOTENDS
