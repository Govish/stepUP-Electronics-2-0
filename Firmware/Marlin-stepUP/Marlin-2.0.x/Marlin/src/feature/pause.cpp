...
#include "../module/temperature.h"
  ...
  #include "leds/printer_event_leds.h"
#endif
...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (fast_load_length) {
    ...
    for (float purge_count = purge_length; purge_count > 0 && wait_for_user; --purge_count)
      do_pause_e_move(10, ADVANCED_PAUSE_PURGE_FEEDRATE);
    wait_for_user = false;
...
/**
 * cold pull filament from the hotend
 *
 * - Fail if the a safe temperature was not reached
 * - Show "wait for unload" placard
 * - Extrude, cool, then unload filament
 * - Disable E stepper (on most machines)
 *
 * Returns 'true' if unload was cold pull was complete, 'false' for abort
 */
bool cold_pull_filament(const bool show_lcd/*=false*/,
                        const PauseMode mode/*=PAUSE_MODE_PAUSE_PRINT*/
                         #if BOTH(FILAMENT_UNLOAD_ALL_EXTRUDERS, MIXING_EXTRUDER)
                           , const float &mix_multiplier/*=1.0*/
                         #endif
  ...
  #if HAS_LCD_MENU
    if (show_lcd) lcd_pause_show_message(PAUSE_MESSAGE_COLDPULL, mode);
  #endif

  // Extrude filament
  do_pause_e_move((COLD_PULL_EXTRUDE_LENGTH) * mix_multiplier, (COLD_PULL_EXTRUDE_FEEDRATE) * mix_multiplier);
  ...
  // Quickly retract
  do_pause_e_move(-(COLD_PULL_INITIAL_RETRACT_LENGTH) * mix_multiplier,
                   (COLD_PULL_INITIAL_RETRACT_FEEDRATE) * mix_multiplier);
  ...
  do_pause_e_move(-(COLD_PULL_UNLOAD_LENGTH) * mix_multiplier, (COLD_PULL_UNLOAD_FEEDRATE) * mix_multiplier);

  #if HAS_LCD_MENU
    if (show_lcd) lcd_pause_show_message(PAUSE_MESSAGE_CONT_PULL); //CHANGE
  #endif

  wait_for_user = true;
  #if ENABLED(HOST_PROMPT_SUPPORT)
    host_prompt_do(PROMPT_USER_CONTINUE, PSTR("Filament Retracting..."), CONTINUE_STR);
  #endif
  #if ENABLED(EXTENSIBLE_UI)
    ExtUI::onUserConfirmRequired_P(PSTR("Filament Retracting..."));
  #endif
  for (float ret_dist = COLD_PULL_MAX_RETRACT; ret_dist > 0 && wait_for_user; --ret_dist)
    do_pause_e_move(-10*(mix_multiplier), (COLD_PULL_UNLOAD_FEEDRATE) * mix_multiplier);
  wait_for_user = false;

  //keep reversing until user says okay
...
bool pause_print(const float &retract, const xyz_pos_t &park_point, const float &unload_length/*=0*/, const bool show_lcd/*=false*/ DXC_ARGS) {
  ...
  #if ENABLED(ALTERNATE_EVENT_LEDS)
    printerEventLEDs.set_paused(true);
    printerEventLEDs.update_auto_leds();
  #endif
...
void resume_print(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  #if ENABLED(ALTERNATE_EVENT_LEDS)
    printerEventLEDs.set_paused(false);
    printerEventLEDs.update_auto_leds();
  #endif
}
